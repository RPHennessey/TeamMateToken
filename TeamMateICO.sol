...
contract TeamMateICO {
    using SafeMath for uint;

    // pre-ICO
      // The maximum amount of tokens to be sold during pre-ICO
      // 162 Million
      uint constant public pre_maxGoal = 162000000e18;
      // There are different prices and amount available in each period
      uint[2] public pre_prices = [30000, 24000];
      uint[1] public pre_amount_stages = [81000000e18]; // the amount available in each stage

      // The start date of the pre-ICO
      // Sunday, 1 April 2018 13:00:00 GMT
      uint constant public pre_start = 1522587600;

      // The end date of the pre-ICO
      // Monday, 30 April 2018 13:00:00 GMT
      uint constant public pre_end = 1525093200;
    ...
    // ICO
      ...
      // 338 Million
      uint constant public maxGoal = 338000000e18;
      // There are different prices and amount available in each period
      uint[1] public prices = [16000];

      // The start date of the crowdsale
      uint constant public start = 1527858000; // Friday, 1 June 2018 13:00:00 GMT
      // The end date of the crowdsale
      uint constant public end = 1531659600; // Sunday, 15 July 2018 13:00:00 GMT
    ...
    uint constant public fundingGoal = 100000000e18; // 10%
    ...
    function TeamMateICO(address tokenAddr, address walletAddr, address tokenOwnerAddr) {
    ...
    function exchange(address receiver) payable {
        ...
        if (isPreICO)
        {
            ...
            if (pre_tokensSold < pre_amount_stages[0])
                require(numTokens <= 6000000e18); // max threshold for pre-ICO: 6mil TMT tokens for stage-I
            else
                require(numTokens <= 12500000e18); // max threshold for pre-ICO: 12.5mil TMT tokens for stage-II
        }
        if (isICO)
        {
